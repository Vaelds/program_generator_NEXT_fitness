    ...
    const blockHTML = (b, bi) => {
      ...
      if (kind === "warmup") {
        ...
        return `<section class="client-block client-warmup">${heading}<p class="client-purpose">Gå fra roligt tempo til større bevægelser, teknikprøve og til sidst højere puls. RPE = din oplevede anstrengelse fra 1 til 10.</p>${groups.length ? groups.map(group => {
          ...
          return `<div class="client-phase"><span>${first ? `${E.stamp(a.start + first.start)}–${E.stamp(a.start + last.end)}` : ""}</span><div><strong>${h(ex.phase)}</strong><p>${group.map(x => `${h(x.ex.clientName || x.ex.name)} (${x.ex.dose.seconds} sek.)`).join(" + ")} · 10 sek. skift efter hver.</p><small>${h(ex.clientCue)}</small></div><b>RPE ${h(ex.targetRpe)}</b></div>`;
        }).join("") : `<p>${h(a.formula)}</p>`}</section>`;
      }
      if (kind === "cooldown") return `<section class="client-block client-cooldown">${heading}<p>${b.exercises.map(ex => `${h(ex.clientName || ex.name)}: ${ex.dose?.seconds || "?"} sek.${ex.transitionSeconds ? ` + ${ex.transitionSeconds} sek. skift` : ""}${ex.clientCue?.includes("pr. side") ? ` (${h(ex.clientCue.split(".").slice(0,2).join("."))})` : ""}`).join(" · ")}</p><p><b>${E.stamp(a.outro || 0)} fælles afslutning:</b> Én observation fra hver: Hvad hjalp makkeren dig med, og hvad justerer du næste gang?</p></section>`;
      const rows = b.exercises.map((ex, i) => {
        const dose = E.doseLabel(ex, b.clock), image = imageURL(ex), video = videoURL(ex), role = kind === "roles" ? `Rolle ${i + 1}` : `Øvelse ${i + 1}`;
        return `<article class="client-exercise"><div class="client-thumb">${image ? `<img src="${h(image)}" alt="${h(ex.name)}">` : `<span>${String(i + 1).padStart(2, "0")}</span>`}</div><div class="client-exercise-copy"><h4><small>${role}</small> ${h(ex.clientName || ex.name)}</h4><p>${h(ex.clientCue || ex.cue)}</p><small><b>Lettere:</b> ${h(ex.regression || "Tilpas med instruktøren")} · <b>Sværere:</b> ${h(ex.progression || "Tilpas med instruktøren")}</small>${ex.load ? `<small><b>Belastning:</b> ${h(ex.load)}</small>` : ""}${ex.notes ? `<small class="client-note">${h(ex.notes)}</small>` : ""}${video ? `<a href="${h(video)}" target="_blank" rel="noopener noreferrer">Video ↗</a>` : ""}</div><div class="client-dose"><b>${dose.mode}</b><strong>${h(dose.text)}</strong>${dose.detail ? `<small>${h(dose.detail)}</small>` : ""}<small>${ex.transitionSeconds ?? b.clock?.transition ?? "?"} sek. skift</small>${ex.targetRpe ? `<small>RPE ${h(ex.targetRpe)}</small>` : ""}</div></article>`;
      }).join("");

(function () {
  ...
  function snapshot() { return { format: "NEXT-training", version: 5, savedAt: new Date().toISOString(), settings: clone(S), meta: clone(meta), pdf: clone(pdf), custom: clone(custom), libraryOverrides: clone(libraryOverrides), favorites: [...favorites] }; }
  ...
  function programText() {
    ...
    })].filter(Boolean).join("\n") + "\n\nMUSIKVALG · " + S.music + "\n" + window.NextMusicData.ordered(S).map(t => `${window.NextMusicData.phases[t.phase]}: ${t.title} – ${t.artist}\n${window.NextMusicData.href(t)}`).join("\n");
  }
  ...
  async function prepareLayout(frame) {
    ...
    if (pdf.layout === "client") {
      ...
      return large.length ? "Lange tekster eller mange øvelser fortsætter på ekstra sider. Alt indhold bevares; kontrollér sideskiftene i udskriftsvinduet." : `Kundeprogram: ${pages.length} A4-sider${pdf.music ? " + 1 side med klikbare Spotify-links" : ""}. Vælg A4, 100 % og slå browserens egne sidehoveder fra.`;
    }
  ...
  function validateSnapshot(raw) {
    if (!raw || raw.format !== "NEXT-training" || ![3, 4, 5].includes(raw.version) || !raw.settings || !Array.isArray(raw.settings.program)) throw Error("Filen er ikke en NEXT-programfil (version 3, 4 eller 5).");
    ...
    settings.musicSelections = window.NextMusicData.sanitize(src.musicSelections);
  ...
  function bindActions() {
    ...
    $("music").addEventListener("change", remember, true);
  ...
  function initialize() {
    ...
    mount(); bindActions(); window.NextPro = { remember, musicMeta: () => ({ ...meta }), blockTools, exerciseHTML, visualHTML, renderPrint, preparePrint, snapshot, validateSnapshot, printProgram, programText };
